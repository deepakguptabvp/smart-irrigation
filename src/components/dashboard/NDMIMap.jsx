...
export default function NDMIMapSatelliteView({
  ...
  const flattenCoords = ndmiBounds?.[0] || [];

  const lats = flattenCoords.map(coord => coord[1]); // latitude is at index 1
  const lngs = flattenCoords.map(coord => coord[0]); // longitude is at index 0
